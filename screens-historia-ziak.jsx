...
function hzFlattenKat(tree, path, out) {
  out = out || [];
  path = path || [];
  for (const node of tree) {
    out.push({ name: node.name, path });
    if (node.children && node.children.length)
      hzFlattenKat(node.children, [...path, node.name], out);
  }
  return out;
}
const HZ_KAT_FLAT = hzFlattenKat(HZ_KAT_TREE);
      ...
      <div style={{
        ...
        {hasKids
          ? <button onClick={() => toggle(node.name)} title={isOpen ? 'Zbaliť' : 'Rozbaliť'} style={{
              ...
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke={dark ? '#7CC7EE' : HZ_ACCENT_DEEP} strokeWidth="2.8" strokeLinecap="round" strokeLinejoin="round">
...
function HzKatSearch({ value, open, onToggle, onPick, dark }) {
  const ink = dark ? '#F2F7FB' : '#1A2B3D';
  const inkSoft = dark ? '#A8B6C8' : '#4A5B6E';
  const line = dark ? '#2A3447' : '#E4EBF2';
  ...
  const nq = hzNorm(q.trim());
  const results = nq
    ? HZ_KAT_FLAT.filter(it => hzNorm(it.name).includes(nq)).slice(0, 40)
    : HZ_KAT_FLAT;
    ...
    <div ref={wrapRef} style={{ position: 'relative' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'stretch', gap: 8 }}>
        <div onClick={() => inputRef.current && inputRef.current.focus()} style={{
          ...
          <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke={panelOpen && mode === 'search' ? (dark ? '#7CC7EE' : HZ_ACCENT_DEEP) : inkSoft} strokeWidth="2.6" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}>
      ...
      {panelOpen &&
        <div style={{
          ...
          <div data-scroll-area onWheel={e => e.stopPropagation()} style={{
            ...
            {mode === 'tree' &&
              HZ_KAT_TREE.map(n =>
                <HzKatNode key={n.name} node={n} depth={0} value={value} expanded={expanded} toggle={toggleNode} onPick={pick} dark={dark} />)}
      ...
      <div style={{
        ...
        <div style={{ padding: '0 18px 10px', display: 'flex', gap: 8, alignItems: 'center' }}>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7, flex: 1, minWidth: 0 }}>
            {[
              { key: 'obdobie', text: obLabel, onClear: () => setStage('filter') },
